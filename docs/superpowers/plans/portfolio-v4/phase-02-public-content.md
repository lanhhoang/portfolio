...
| `db/migrate/*_create_{projects,posts,tags,profiles,resumes}.rb`                                                                                       | One generated migration for each locale-independent content model                       |
| `db/migrate/*_create_{project_translations,post_translations,tag_translations,profile_translations,resume_translations}.rb`                            | One generated migration for each localized content model                                |
| `db/migrate/*_create_taggings.rb`                                                                                                                     | Generated polymorphic join-table migration                                               |
...
- Generate: eleven `db/migrate/*_create_*.rb` files, one for each application model
...
- [ ] **Step 1: Generate Active Storage plus one migration per application model**

```bash
bin/rails active_storage:install
bin/rails g model Project role:string started_on:date ended_on:date live_url:string source_url:string featured_position:integer --skip-test-framework
bin/rails g model Post --skip-test-framework
bin/rails g model Tag --skip-test-framework
bin/rails g model Profile public_contact_email:string social_links:json accent:string singleton_guard:integer --skip-test-framework
bin/rails g model Resume updated_on:date singleton_guard:integer --skip-test-framework
bin/rails g model ProjectTranslation project:references locale:string title:string slug:string summary:text body_markdown:text body_html:text search_text:text state:string scheduled_at:datetime published_at:datetime --skip-test-framework
bin/rails g model PostTranslation post:references locale:string title:string slug:string excerpt:text body_markdown:text body_html:text search_text:text state:string scheduled_at:datetime published_at:datetime --skip-test-framework
bin/rails g model TagTranslation tag:references locale:string name:string slug:string --skip-test-framework
bin/rails g model Tagging tag:references 'taggable:references{polymorphic}' --skip-test-framework
bin/rails g model ProfileTranslation profile:references locale:string display_name:string headline:string introduction:text biography_markdown:text biography_html:text availability_label:string --skip-test-framework
bin/rails g model ResumeTranslation resume:references locale:string title:string description:text --skip-test-framework
bin/rails g test_unit:model public_content --no-fixture
```

Expected: Rails generates one untouched Active Storage migration, eleven ordered application migrations, the eleven `ApplicationRecord` subclasses, and `test/models/public_content_test.rb`. Parent models are generated before their dependent translations and taggings. `--skip-test-framework` prevents redundant fixtures and empty per-model tests.
...
Expected: FAIL with `ActiveRecord::PendingMigrationError`; the red test confirms the generated migrations have not been applied implicitly.
...
- [ ] **Step 5: Fill each generated model migration with its exact constraints and indexes**

Replace each generated migration body with the matching single-table definition below. Keep the generator-assigned timestamp and class name.
...
`*_create_project_translations.rb`:
...
`*_create_profile_translations.rb`:
...
`*_create_resume_translations.rb`:
...
The composite unique indexes are the authoritative race-safe constraints. The listing indexes match `publicly_visible` ordering. Each application migration creates exactly one table and reverses with its generated `change` method. Parent migrations precede dependent migrations, so rollback must remain newest-first. SQLite cannot add a foreign key to a polymorphic target, so `taggings.taggable_type/id` is protected by model associations and the unique index.

Apply the atomic migrations, then rerun the red test:
...
Expected: migration succeeds; the test now fails on missing associations, attachment declarations, scopes, or validations rather than pending schema.
...
- [ ] **Step 8: Prove atomic rollback and run the focused test**

Run:

```bash
bin/rails db:migrate
bin/rails db:rollback STEP=11
bin/rails db:migrate
bin/rails test test/models/public_content_test.rb
```

Expected: the first migrate is a no-op, rollback removes exactly the eleven application tables while leaving Active Storage installed, remigration recreates the schema in dependency order, and all 7 tests pass.
