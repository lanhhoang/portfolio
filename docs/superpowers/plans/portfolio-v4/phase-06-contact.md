...
- Create: `test/integration/public_contact_messages_test.rb`
...
```ruby
# test/integration/public_contact_messages_test.rb
require "test_helper"
...
Run: `bin/rails test test/integration/public_contact_messages_test.rb`
...
The honeypot is checked before save, and `website` is never permitted or persisted. The rate limit uses the existing Rails cache rather than a dependency. `config/environments/test.rb` already uses `:memory_store`; do not edit it.

- [ ] **Step 5: Filter the body parameter and add the localized view**

Append `:body` without replacing the existing list, which includes the Phase 3 `:recovery` filter:
...
<% content_for :title, "#{t("pages.contact.title")} — #{t("site.name")}" %>

<main class="mx-auto w-full max-w-3xl px-4 py-12 sm:px-6" aria-labelledby="contact-title">
  <p class="text-sm font-semibold uppercase tracking-widest text-accent"><%= t("pages.contact.eyebrow") %></p>
...
- [ ] **Step 7: Update the contact navigation target**

In `app/views/shared/_header.html.erb`, change only the contact entry:

```erb
["public/contact_messages", nil, t("navigation.contact"), localized_contact_path(locale: current_locale)]
```

- [ ] **Step 8: Run focused public tests**

Run: `bin/rails test test/integration/public_contact_messages_test.rb test/models/contact_message_test.rb test/jobs/contact_notification_job_test.rb`
...
- Create: `app/controllers/admin/messages/states_controller.rb`
- Create: `app/controllers/admin/messages/delivery_retries_controller.rb`
...
- Create: `test/integration/admin_messages_test.rb`
...
```ruby
# test/integration/admin_messages_test.rb
require "test_helper"
...
Run: `bin/rails test test/integration/admin_messages_test.rb`
...
- [ ] **Step 6: Add navigation, actionable dashboard counts, and a usable desktop layout**
...
Add the inbox link beside the existing admin resource links in `app/views/layouts/admin.html.erb`:

```erb
<li><%= link_to "Messages", admin_messages_path, class: "admin-action" %></li>
```

The layout has no sidebar, so remove the unused desktop grid columns from its outer main element:
...
Run: `bin/rails test test/integration/admin_messages_test.rb test/models/contact_message_test.rb test/jobs/contact_notification_job_test.rb`
...
```bash
git add config/routes.rb app/controllers/admin/messages_controller.rb app/controllers/admin/messages app/views/admin/messages app/controllers/admin/dashboard_controller.rb app/views/admin/dashboard/show.html.erb app/views/layouts/admin.html.erb test/integration/admin_messages_test.rb
...
2. Development request logs do not contain the submitted body.
3. A failed row exposes **Retry email** in `/admin/messages`; two rapid retry submissions enqueue once and preserve the sender, subject, body, and original `created_at`.
4. At 320 CSS pixels and 200% zoom, form fields, cards, message text, and action buttons do not overflow horizontally; every action works by keyboard and touch without hover.
5. At desktop width, the inbox uses the available content width rather than the removed 14rem grid column.
...
Do not create the tag until every automated command and all five manual observations pass.

## Phase 6 Acceptance Checklist

- [ ] Contact rows enforce required fields, bounded lengths, valid enums, and database state constraints.
- [ ] New valid messages enqueue one ID-only job after commit; enqueue failure preserves the committed row as failed; invalid, honeypot, and throttled submissions enqueue none.
...
- [ ] Retry atomically changes failed-to-pending once, enqueues once, and never edits the original message content.
